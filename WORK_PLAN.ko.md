...
- 원격: `https://github.com/olive-su/UnitConverter_02.git`. 로컬 브랜치: `red` (사이클 2 — U-IN-01 RED 완료).
...
| 4 — GREEN | `green` | Report 07~12 | **사이클 1 완료** — D-CNV+Golden (PR [#6](https://github.com/olive-su/UnitConverter_02/pull/6) open) |
| 5 — REFACTOR | `refactor` | Report 13~14 | **사이클 1 완료** — P0 `6219a81` (푸시됨; PR 대기) |
| 6 — Repeat | `red`→`green`→`refactor` | Report 15+ | **사이클 1 완료** · **사이클 2 진행 중** — U-IN-01 RED |
...
| 묶음 | Test ID | RED | GREEN | REFACTOR | Report |
|------|---------|-----|-------|----------|--------|
| 1 | U-IN-01 빈 입력 → format error | **완료** | **다음** | — | 15 |
| 2 | U-IN-02 콜론 없음 | 대기 | — | — | — |
| 3 | U-IN-03 음수 거부 | 대기 | — | — | — |
| 4 | U-OUT-01 출력 라인 | 대기 | — | — | — |
...
- **진행**: **사이클 1 완료** (D-CNV RED+GREEN+Golden+REFACTOR P0). **사이클 2 시작** — U-IN-01 RED 완료.
- **로컬 브랜치**: `red`. 열린 PR: #2, #4, #6; `refactor` @ `6219a81` 푸시됨 (PR 대기).
- **다음 실행**: U-IN-01 **GREEN** on `green` — `/green-minimal` → `parse_input("")` → `FormatError`.
